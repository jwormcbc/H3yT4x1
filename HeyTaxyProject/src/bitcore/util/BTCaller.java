...
public class BTCaller {
	...
	private final static String URL = "http://192.168.0.227:8084/HT/ServicioTaxero?wsdl";
	...
	public HashMap<String, Taisi> callWSServicioTaxero_TaxByExt(final String elegido,final String pasajero){
        final String NAMESPACE = "http://servicios.bitcore/";
        final String METHOD_NAME = "TaxByExt_elegidoYtaxitoYo";
        final String SOAP_ACTION = "http://servicios.bitcore/"+METHOD_NAME;
        final HashMap<String, Taisi> hmTEYT=new HashMap<String, Taisi>();
        ...
        Thread networkThread = new Thread() {
            ...
            public void run() {
              try {
                 ...
                 request.addProperty("minx", "-120");
                 request.addProperty("maxx", "-80");
                 request.addProperty("elegido",elegido);
                 request.addProperty("taxitoYo",pasajero);
                 envelope.setOutputSoapObject(request);
                 ...
                 final String str = response.toString();
                 //Log.d("call",str);
            	 Log.d("Out", "EYTY  call: " +str);// taxis.get(String.valueOf(i)).getNombre());
                 String diviCadena[]=str.split(",");
                 String divSubCadena[];
                 int t=1;
                 for(String s:diviCadena){
             	 	divSubCadena=s.split(":");
             	 	hmTEYT.put(String.valueOf(t),//id del hash map
             	 			new Taisi(  //contruimos el obj taisi
             	 		    divSubCadena[0],  //primer valor es el id
             	 			new LatLng(Double.parseDouble(divSubCadena[2].toString()), Double.parseDouble(divSubCadena[3].toString())),  //elementos 2 y 3 son lat lon
             	 			divSubCadena[1]  // nombre, es el arreglo 1 pero en el constructor se pone al final
